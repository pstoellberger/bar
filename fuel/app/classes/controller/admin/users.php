...
class Controller_Admin_Users extends Controller_Admin {
	...
	public function action_exportunpaid(){
		...
		$query=DB::query('select u.is_team, firstname,lastname, saldo, i.title, min(i.price), count(i.title), round(sum(c.price),2) as total  from users u, consumptions c, items i  where u.id = c.user_id and c.item_id = i.id and saldo > 0 and u.status <> 1 and c.status = 1 group by 1,2,3,4,5');
		$rows=$query->execute()->as_array();
		
		echo "is_team;firstname;lastname;unpaid_saldo;item;item_single_price;item_count;item_total\n";
		foreach ($rows as $row):
